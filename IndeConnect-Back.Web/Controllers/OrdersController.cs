...
public class OrdersController : ControllerBase
{
    ...
    public async Task<ActionResult<OrderDto>> CreateOrder([FromBody] CreateOrderRequest request)
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500,
                new { message = "Erreur lors de la création de la commande", detail = ex.Message });
        }
    }
    ...
    /// GET /indeconnect/orders/{orderId}/tracking
    /// </summary>
    [HttpGet("{orderId}/tracking")]
    public async Task<ActionResult<OrderTrackingDto>> GetOrderTracking(long orderId)
    {
        var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        if (string.IsNullOrEmpty(userIdClaim) || !long.TryParse(userIdClaim, out var userId))
        {
